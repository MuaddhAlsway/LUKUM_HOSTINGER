<?php
/**
 * LAKUM Artspace - Update Press API
 * Updates an existing press release in the database
 */
...
try {
    ...
    if ($source !== null) { $updates[] = 'source = ?'; $params[] = $source; $types .= 's'; }
    if ($cover_image !== null) { $updates[] = 'cover_image = ?'; $params[] = $cover_image; $types .= 's'; }
    if ($press_date !== null) { $updates[] = 'press_date = ?'; $params[] = $press_date; $types .= 's'; }
    ...
    $query = 'UPDATE press SET ' . implode(', ', $updates) . ' WHERE id = ?';
    ...
    error_log('Update Press Error: ' . $e->getMessage());
