 ...
 * FIXED: Uses correct IDs (12-19) from database
 */
...
try {
    ...
    // REAL Arabic translations - using correct IDs from database (12-19)
    $arabicTranslations = [
        12 => [
    ...
    foreach ($arabicTranslations as $press_id => $translations) {
        ...
        if (!$update_stmt) {
            $errors[] = "Prepare failed for ID $press_id";
            continue;
        }
        ...
        if ($update_stmt->execute()) {
            ...
            $errors[] = "Execute failed for ID $press_id";
        }
    }
    ...
    echo json_encode([
        ...
        'updated' => $updated,
        'errors' => $errors,
        'note' => 'Arabic translations added. Press cards will display Arabic when ?lang=ar'
    ]);
